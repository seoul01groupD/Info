...
# 알면 좋은 것:
## 얕은 복사와 깊은 복사
- 얕은 복사: 복사를 했을 때 한 개가 변화하면 나머지 한 개도 변화하는 것
- 깊은 복사: 복사를 하고 한 개가 변화해도 나머지 한 개는 변화하지 않는 것
- 원래 변경이 가능한 리스트, 집합, 딕셔너리 등을 복사할 때 얕은 복사가 나타나고
...
- 리스트의 경우 `리스트명.copy(0)` ,`리스트명[:]`로 복사 가능(리스트 슬라이싱이 deepcopy보다 속도가 빠르다)
